...
function resolveTimelineDateFromPoint(slotEl, clientX, clientY = null, gridRoot = null, debugHook = null) {
  ...
  if (Number.isFinite(clientX) && Number.isFinite(clientY)) {
    ...
    const hitTimeline = hitEl?.closest?.(".schedule-timeline[data-date]");
    if (hitTimeline && grid.contains(hitTimeline)) {
      pointHit = String(hitTimeline.dataset.date || "").trim() || null;
    }
  }
  if (pointHit) {
...
function renderAppliedExperiments() {
  ...
  const applied = entries.map(([uid, record]) => {
    ...
    return {
      uid,
      name: exp?.name || record?.name || record?.experiment_type || uid,
      type: exp?.type || record?.type || record?.experiment_type || "-",
      location: exp?.location || record?.location || "",
      reward: exp?.reward || record?.reward || "",
      durationMin: exp?.duration_min || record?.duration_min || "",
      contact: exp?.contact_phone || record?.contact_phone || "",
      notes: exp?.notes || record?.notes || "",
      deviceHint: exp?.device_restriction_hint || record?.device_restriction_hint || "",
